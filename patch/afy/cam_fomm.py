...
def kp_to_pixels(arr):
    ...
    return ((arr + 1) * (IMG_SIZE // 2 - 1)).astype(np.int32)
...
def draw_landmark_text(frame, thk=2, fontsz=0.5, color=(0, 0, 255)):
    frame = frame.copy()
    cv2.putText(frame, "ALIGN FACES", (int(60 * IMG_SIZE / 256), int(20 * IMG_SIZE / 256)), 0, fontsz * IMG_SIZE / 255, color, thk)
    cv2.putText(frame, "THEN PRESS X", (int(60 * IMG_SIZE / 256), int(245 * IMG_SIZE / 256)), 0, fontsz * IMG_SIZE / 255, color, thk)
    return frame


def draw_calib_text(frame, thk=2, fontsz=0.5, color=(0, 0, 255)):
    frame = frame.copy()
    cv2.putText(frame, "FIT FACE IN OVAL", (int(60 * IMG_SIZE / 256), int(20 * IMG_SIZE / 256)), 0, fontsz * IMG_SIZE / 255, color, thk)
    cv2.putText(frame, "W - ZOOM IN", (int(60 * IMG_SIZE / 256), int(40 * IMG_SIZE / 256)), 0, fontsz * IMG_SIZE / 255, color, thk)
    cv2.putText(frame, "S - ZOOM OUT", (int(60 * IMG_SIZE / 256), int(60 * IMG_SIZE / 256)), 0, fontsz * IMG_SIZE / 255, color, thk)
    cv2.putText(frame, "THEN PRESS X", (int(60 * IMG_SIZE / 256), int(245 * IMG_SIZE / 256)), 0, fontsz * IMG_SIZE / 255, color, thk)
    return frame
...
if __name__ == "__main__":
    ...
    if opt.is_worker:
        ...
        if opt.mode == "face-animation":
            from afy import predictor_face_animation
            predictor = predictor_face_animation.PredictorFaceAnimation(
                **predictor_args
            )
        else:
